...
export const blendVideoWithAudio = (
  ...
  let command = `-i ${videoStream} -i ${audioStream} -shortest ${newSource}`;

  if (videoRate) {
    command = `-i ${videoStream} -i ${audioStream} -filter:v "setpts=PTS/${videoRate}" -shortest ${newSource}`;
  }
...
export const compressVideo = (sourceUri, callback) => {
  ...
  RNFFmpeg.execute(`-i ${sourceUri} -c:v mpeg4 ${newSource}`).then((result) => {
    console.log('  did compressVideo  newSource++++', newSource);
    callback(newSource);
  });
};
